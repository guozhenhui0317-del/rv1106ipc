...
int rkipc_rockiva_init() {
	...
	/* 本项目允许通过 INI 指定官方模型目录，默认值仍兼容原 RKIPC Demo。 */
	snprintf(globalParams.modelPath, ROCKIVA_PATH_LENGTH, "%s",
	         rk_param_get_string("rockiva:model_path", "/usr/lib/"));
	globalParams.coreMask = 0x04;
	globalParams.logLevel = ROCKIVA_LOG_ERROR;
	model_type = rk_param_get_string("event.regional_invasion:rockiva_model_type", "small");  // ini文件内容解析成字典，从字典中获取键的对值
	...
	if (rk_param_get_int("video.source:rotate_in_venc", 0)) // 设置旋转角度
		rotation = 0;
	/* 0 度必须保持原图方向；不能沿用旧 Demo 中 0/180 都映射为 180 的行为。 */
	if (rotation == 0) {
	...
	if (ret != ROCKIVA_RET_SUCCESS) {
		...
		/* BA 未创建，但基础 RockIVA handle 已创建，必须在返回前释放。 */
	...
	if (!rockiva_signal) {
		...
		/* 信号量是 PushFrame 等待释放回调的前提；失败时回滚两级 handle。 */
...
int rkipc_rockiva_write_rgb888_frame(uint16_t width, uint16_t height, uint32_t frame_id,
	...
	int ret;
	/* 先检查运行状态再分配，避免停止阶段的送帧调用泄漏 image。 */
	if (!rockit_run_flag)
		return 0;
	RockIvaImage *image = (RockIvaImage *)malloc(sizeof(RockIvaImage));
	/* RockIVA 只在 PushFrame 调用期间读取描述符；回调后即可释放该小对象。 */
	if (!image)
		return -1;
...
int rkipc_rockiva_write_rgb888_frame_by_fd(uint16_t width, uint16_t height, uint32_t frame_id,
                                           int32_t fd) {
	int ret;
	/* FD 由调用者持有，本函数只构造并提交 RockIvaImage 描述符。 */
	if (!rockit_run_flag)
		return 0;
	RockIvaImage *image = (RockIvaImage *)malloc(sizeof(RockIvaImage));
	if (!image)
...
int rkipc_rockiva_write_nv12_frame_by_fd(uint16_t width, uint16_t height, uint32_t frame_id,
                                         int32_t fd) {
	int ret;
	/* media.cpp 的 VI1 推理线程走此零拷贝入口；停止后不再分配临时对象。 */
	if (!rockit_run_flag)
		return 0;
	RockIvaImage *image = (RockIvaImage *)malloc(sizeof(RockIvaImage));
	if (!image)
...
int rkipc_rockiva_write_nv12_frame_by_phy_addr(uint16_t width, uint16_t height, uint32_t frame_id,
                                               uint8_t *phy_addr) {
	int ret;
	/* 保留官方物理地址入口供兼容调用，本项目正常路径使用上面的 DMABUF fd。 */
	if (!rockit_run_flag)
		return 0;
	RockIvaImage *image = (RockIvaImage *)malloc(sizeof(RockIvaImage));
	if (!image)
