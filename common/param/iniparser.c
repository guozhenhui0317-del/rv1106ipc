...
static unsigned strstrip(char *s) {
	...
	last = s + strlen(s);
	while (isspace((int)*s) && *s) // 只要存在标准空白字符就继续往后走
		s++;
	while (last > s) {
...
dictionary *iniparser_load(const char *ininame) {
	...
	if ((in = fopen(ininame, "r")) == NULL) {  // 判断文件是否打开，未打开就报错
		iniparser_error_callback("iniparser: cannot open %s\n", ininame);
		return NULL;
	}

	dict = dictionary_new(0);  // 创建字典，当传入 0 时，字典默认分配 128 个槽位
	if (!dict) {  // 字典创建失败就返回NULL
		fclose(in);
		return NULL;
	}
	...
	last = 0;
	// line + 0： 指向数组的第0个元素：line[0]
	while (fgets(line + last, ASCIILINESZ - last, in) != NULL) { // 从文件中读取ASCIILINESZ - last个字符到line数组中，返回值为NULL表示读取失败
		lineno++; // 统计行号
		len = (int)strlen(line) - 1; // 最后一个字符的下表
		if (len <= 0)
			continue;
		/* Safety check against buffer overflows */
		// feof()函数用于检查文件结束标志是否被设置，返回非零值表示文件结束，返回0表示未结束
		if (line[len] != '\n' && !feof(in)) { // 说明这一行太长
			iniparser_error_callback("iniparser: input line too long in %s (%d)\n", ininame,
			                         lineno);
			dictionary_del(dict); // 删除解析的配置
			fclose(in);
			return NULL;
		}
